...
select 
    case
        when buseo = '영업부' then 0.05
        when buseo = '총무부' then 0.03
        when buseo = '개발부' then 0.07
        else 0.02
    end
    from tblInsa;
...
select 
    case
        when length(first) >= 3 then '긴 이름'
        else '짧은 이름'
    end
    from tblComedian;
    
-- 15. 코미디언의 나이(2024년 기준)를 계산하여 30세 이상이면 '중견', 그 외는 '신인'으로 분류하세요.
-- tblComedian 에는 나이가 없음
select 
    case
        when TO_NUMBER(substr(ssn, 0, 2)) <= 95 then '중견'
        else '신인'
    end
    from tblInsa;
