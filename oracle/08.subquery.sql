...
select 
    name, height, weight,
    (select name from tblWomen where couple = tblMen.name),
...
select 
    ...
    f.name, f.height, f.weight
from tblMen m inner join tblWomen f on m.couple = f.name;
...
select * from tblAddressBook where job = 
...
--    8.7%        30.7%        28.3%        32.2%
select 
    round((select count(*) from tblAddressBook where job = m.job and floor(age / 10) = 1) / count (*) * 100, 1) || '%' "[10대]",
    round((select count(*) from tblAddressBook where job = m.job and floor(age / 10) = 2) / count (*) * 100, 1) || '%' "[20대]",
    round((select count(*) from tblAddressBook where job = m.job and floor(age / 10) = 3) / count (*) * 100, 1) || '%' "[30대]",
    round((select count(*) from tblAddressBook where job = m.job and floor(age / 10) = 4) / count (*) * 100, 1) || '%' "[40대]"
from tblAddressBook m
    where job = (select job from tblAddressBook group by job 
        having count(*) = (select max(count(job)) from tblAddressBook group by job))
        group by job;

select 
    floor(age / 10) || '0대' 나이대, 
    round(count(*) / (select count(*) from tblAddressBook where job = (select job from tblAddressBook group by job 
        having count(*) = (select max(count(job)) from tblAddressBook group by job))) * 100, 1) || '%' 확률 
from tblAddressBook
    where job = (select job from tblAddressBook group by job 
        having count(*) = (select max(count(job)) from tblAddressBook group by job))
    group by floor(age / 10) order by 나이대;
