...
select distinct v.name 비디오제목, 
    v.qty - (select count(*) from tblRent where video = v.seq and retdate is null) 보유수량, 
    g.price 대여가격 from tblVideo v
...
select e.* from employees e
    inner join (select j.job_id job_id, max(e.salary) salary from employees e 
        left outer join jobs j 
            on e.job_id = j.job_id
                group by j.job_id) ej
                    on e.job_id = ej.job_id and e.salary = ej.salary;
...
select first_name || ' ' || last_name, salary, a.department_id from employees a
    inner join (select department_id from employees where salary >= 12000 group by department_id) b 
...
select e.first_name || ' ' || e.last_name 이름, d.department_id 부서번호, d.department_name 부서이름 from employees e
    left outer join departments d
        on e.department_id = d.department_id
            right outer join (select * from locations where lower(city) = 'seattle') l
                on d.location_id = l.location_id;
-- ******************************************************************************************직위가 없는데요
...
select e.* from employees e
    inner join departments d
        on e.department_id = d.department_id
            inner join (select * from employees where lower(first_name) = 'jonathon') e2
...
-- 21. employees. 사원번호와 사원이름, 
--    그리고 그 사원을 관리하는 관리자의 사원번호와 사원이름을 출력하되 각각의 컬럼명을 '사원번호', '사원이름', '관리자번호', '관리자이름'으로 하여 가져오시오.
select * from employees;

select e.employee_id 사원번호, e.first_name || ' ' || e.last_name 사원이름,
    m.employee_id 관리자번호, m.first_name || ' ' || m.last_name 관리자이름 from employees e
...
select to_char(e.hire_date, 'yyyy') 입사년도, avg(salary) from employees e
    inner join jobs j
        on e.job_id = j.job_id
            where lower(j.job_title) = 'sales manager'
...
select city 도시명, round(avg(salary), 2) 평균연봉, count(*) 사원수 from employees e
    inner join departments d
        on e.department_id = d.department_id
...
select e.employee_id 사번, e.first_name || ' ' || e.last_name 이름 from (select * from jobs where lower(job_title) = 'public accountant') j 
    inner join job_history h
