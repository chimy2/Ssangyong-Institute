...
-- WHERE절 > 레코드 필터 역할(가로)
-- SELECT절 > 컬럼 필터 열할(세로)
select * from tblCountry where 조건;
...
select * from tblCountry where area >= 100 and area < 500;
...
select * from tblInsa where basicpay between 1020000 and 1050000;
...
select name from tblInsa where name like '%길%';  -- 길, 김길, 김수길
...
-- 인구수가 미기재된 나라
select * from tblCountry where population == null;  //null을 피연산자로 넣으면 100% false > 어떤 결과도 return 하지 않음
