...
select name, buseo, rownum from tblInsa where rownum between 11 and 20;
select name, buseo, rownum from tblInsa where rownum = 1;
...
select name, buseo, rownum  -- 3. 읽기
    from tblinsa            -- 1. 할당
    where rownum = 3;       -- 2. 조건
-- 한 행씩 조건이 실행되면서 행이 빠질 때마다 rownum이 갱신되기 때문에 평생 3이 올 수 없다
...
select b.* from (select a.*, rownum rnum
    from (select name, basicpay from tblInsa order by basicpay desc) a) b
