/*
    ...
    ---------------------------------------------------------이후부터 오라클에 맞게 적용하여 설계
    - 물리 > 물리적 식별자 + 실제 DBMS에 맞는 세부 설정
    
    Relation        > 관계 > 테이블
    Relationship    > 관계 > 테이블 간의 관계
    
    1. ERD, Entity Relationship Diagram
    - 엔티티(테이블)간의 관계를 표현한 그림
    - 데이터베이스 모델링 기법 중 하나
    - 손, 오피스, 전문툴(eXERD, ER-Win, 온라인 툴(erd cloud) 등)
    
    2. Entity
    - 다른 Entity와 분류(구분)될 수 있고, > 식별 가능
        다른 Entity에 대해 정해진 관계를 맺을 수 있는 데이터 단위
        
    - 릴레이션(데이터 베이스) = 엔티티(데이터 모델링) = 레코드(실제 DB) = 인스턴스 = 객체 >>> 집합(=테이블, 클래스)
        a. 학생 정보 관리
    ...
    - erd에서는 N:N이 오면 안된다. > 실제 쿼리문을 작성할 때 식별할 수 없어 곤란함
    
    정규화, Normalization
    - 자료의 손실이나, 불필요한 정보를 없애고, 데이터 일관성을 유지하며
        데이터 종속성을 최소화해주기 위해서 ERD를 수정하는 작업
    - 우리가 만든 ERD > 비정령 상태, 비정규화 상태 > 정규화 상태
    - 제 1정규화, 제 2정규화, 제 3정규화 등.. > 꼭 순서대로 하지는 않음
    ...
    이상 현상, Anomoly
    1. 삽입 이상
        - 특정 테이블에 데이터를 삽입할 때 원하지 않는 데이터까지 같은 넣어야 하는 상황
    2. 갱신 이상
