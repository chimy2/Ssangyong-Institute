...
<%
	...
	*/
	...
	// ex. C:\class\code\server\JSPTest\src\main\webapp\files
	
	//String path = "C:\\class\\code\\server\\JSPTest\\src\\main\\webapp\\files";
	// 이렇게 경로를 설정하고 바로 업로드 후 다운로드 받으려면 불가능
	// 이클립스 새로고침해야 파일이 폴더 안에 있다는게 확인되기 때문에
	
	// 실제경로 > 진짜 실행경로 > 톰캣이 관리하는 워크스페이스에 존재하는 파일 경로 > 새로고침 필요없이 바로바로 인식 가능
	...
	// 2. 업로드 파일의 최대 크기 지정
	// - 바이트 단위
	int size = 1024 * 1024 * 100; // 100MB > 초과되면 에러발생
...
<body>
	...
	<!-- 
		장점: 평범한 다운로드 제공(모든 파일 다운로드, 이름 원본 유지)
