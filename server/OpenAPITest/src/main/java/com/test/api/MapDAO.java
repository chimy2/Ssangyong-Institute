...
//				레코드 1줄 > DTO 1개
	...
	public ArrayList<FoodDTO> listfood() {
		try {
//			Food + Category
			String sql = "select f.seq as fseq, f.name as fname, f.lat, f.lng, f.address, f.star, f.menu, "
					+ "c.seq as cseq, c.name as cname, c.img from tblFood f inner join tblCategory c on f.category = c.seq order by f.seq desc";
